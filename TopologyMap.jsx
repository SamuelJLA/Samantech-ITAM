...
// --- COMPONENTE: NODO REACTIVO ---
const DeviceNode = ({ data }) => {
  const config = ICON_CONFIG[data.category] || { icon: Box, color: "text-gray-400", bg: "bg-white/5" };
  const Icon = config.icon;
  
  // AHORA RESPONDE AL STATUS REAL
  const isOffline = data.status === 'offline';
  const isCritical = data.is_critical;

  return (
    <div className={`
      p-4 rounded-[2rem] border backdrop-blur-xl shadow-2xl min-w-[170px] transition-all duration-500
      ${isOffline 
        ? isCritical 
          ? 'border-red-600 bg-red-600/20 shadow-[0_0_30px_rgba(220,38,38,0.5)] animate-pulse' // CRÍTICO CAÍDO
          : 'border-orange-500 bg-orange-500/10 shadow-[0_0_20px_rgba(249,115,22,0.3)]'       // NORMAL CAÍDO
        : `border-white/10 ${config.bg} hover:border-white/20`}
    `}>
      <Handle type="target" position={Position.Top} className={`w-3 h-3 border-2 border-white ${isOffline ? 'bg-red-500' : 'bg-blue-500'}`} />
      
      <div className="flex items-center gap-4">
        <div className={`p-3 rounded-2xl border border-white/5 shadow-inner ${isOffline ? 'bg-red-500/20' : config.bg}`}>
          <Icon className={isOffline ? 'text-red-500' : config.color} size={22} />
        </div>
        
        <div className="flex flex-col">
          <div className="flex items-center gap-1">
             <p className={`text-[10px] font-black uppercase tracking-tighter truncate w-24 ${isOffline ? 'text-red-100' : 'text-white'}`}>
              {data.label}
            </p>
            {isCritical && <ShieldCheck size={10} className="text-rose-500" />}
          </div>
          <p className={`text-[7px] font-black uppercase tracking-widest ${isOffline ? 'text-red-400' : config.color} opacity-80`}>
            {isOffline ? '⚠ CONNECTION LOST' : data.category}
          </p>
        </div>
      </div>
