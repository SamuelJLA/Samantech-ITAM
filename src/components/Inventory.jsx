...
import { 
  ...
  Clock, Building2, Share2, HardDrive, ShieldCheck
} from 'lucide-react'
...
export default function Inventory({ companyId, session }) {
    ...
    if (editingAssetId) {
      ...
      if (!updateError) {
        await supabase.from('asset_history').insert([{
          ...
          description: `Actualización técnica integral: ${assetData.name}`,
  ...
  return (
    <div className="p-8 space-y-6 relative min-h-screen">
      ...
      {/* MODAL DE REGISTRO / EDICIÓN (VUELVE HARDWARE Y MAC) */}
      {isModalOpen && (
          ...
          <form onSubmit={handleSaveAsset} className="relative bg-gray-900 border border-gray-800 w-full max-w-2xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="p-10 max-h-[70vh] overflow-y-auto space-y-10 custom-scrollbar">
              ...
              <div className="space-y-6 pt-6 border-t border-white/5">
                ...
                <input className="w-full bg-black/50 border border-gray-800 rounded-2xl p-4 text-white text-xs font-bold" placeholder="Usuario Actual / Asignado" value={newAsset.last_user} onChange={e => setNewAsset({...newAsset, last_user: e.target.value})} />
              </div>

              {/* 3. HARDWARE (VUELVE A LA VIDA) */}
              <div className="space-y-6 pt-6 border-t border-white/5">
                <div className="flex items-center gap-2 text-purple-500 px-2"><HardDrive size={16} /> <span className="text-[10px] font-black uppercase tracking-[0.2em]">Especificaciones Técnicas</span></div>
                <div className="grid grid-cols-2 gap-5">
                  <input className="bg-black/50 border border-gray-800 rounded-2xl p-4 text-white text-xs font-bold" placeholder="Procesador (CPU)" value={newAsset.cpu} onChange={e => setNewAsset({...newAsset, cpu: e.target.value})} />
                  <input className="bg-black/50 border border-gray-800 rounded-2xl p-4 text-white text-xs font-bold" placeholder="Memoria RAM" value={newAsset.ram} onChange={e => setNewAsset({...newAsset, ram: e.target.value})} />
                  <input className="bg-black/50 border border-gray-800 rounded-2xl p-4 text-white text-xs font-bold" placeholder="Almacenamiento" value={newAsset.storage} onChange={e => setNewAsset({...newAsset, storage: e.target.value})} />
                  <input className="bg-black/50 border border-gray-800 rounded-2xl p-4 text-white text-xs font-bold" placeholder="Sistema Operativo" value={newAsset.os} onChange={e => setNewAsset({...newAsset, os: e.target.value})} />
                </div>
              </div>

              {/* 4. OBSERVACIONES */}
              <div className="space-y-4 pt-6 border-t border-white/5">
                <div className="flex items-center gap-2 px-2 text-yellow-500"><ShieldCheck size={16}/><h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Notas Adicionales</h4></div>
                <textarea className="w-full bg-black/50 border border-gray-800 rounded-3xl p-5 text-white text-xs font-bold h-24 resize-none" placeholder="Seriales, marcas, historial de fallas..." value={newAsset.notes} onChange={e => setNewAsset({...newAsset, notes: e.target.value})} />
              </div>
            ...
            <div className="p-8 bg-black/40 border-t border-white/5">
              <button type="submit" className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-5 rounded-[2rem] shadow-xl transition-all active:scale-95 uppercase text-[11px] tracking-widest">
                {editingAssetId ? 'Actualizar Ficha Técnica' : 'Registrar en Inventario'}
              </button>
            </div>
          </form>
        </div>
      ...
      {deleteConfirm.isOpen && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setDeleteConfirm({ isOpen: false, id: null, name: '' })}></div>
          <div className="relative bg-gray-900 border border-red-500/20 w-full max-w-sm rounded-[3rem] shadow-2xl p-10 text-center animate-in zoom-in-95 duration-200">
            <div className="bg-red-500/10 w-20 h-20 rounded-[2.5rem] flex items-center justify-center mx-auto mb-6"><Trash2 className="text-red-500" size={40} /></div>
            <h3 className="text-white text-xl font-black italic uppercase mb-2">Eliminar Activo</h3>
            <div className="grid grid-cols-2 gap-4 mt-8">
              <button onClick={() => setDeleteConfirm({ isOpen: false, id: null, name: '' })} className="bg-gray-800 text-gray-400 font-black py-4 rounded-2xl text-[9px] uppercase">Volver</button>
              <button onClick={confirmDelete} className="bg-red-600 text-white font-black py-4 rounded-2xl text-[9px] uppercase">Eliminar</button>
            </div>
          </div>
        </div>
      )}
